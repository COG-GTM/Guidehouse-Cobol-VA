...
| 9 | Srinjoy | Can your system resolve dependencies across copybooks, includes, and dynamically referenced components (e.g., conditional includes, runtime program calls)? | Use `analysis/dependency-map.md`; ~~identify missing `DATECONV-*` copybooks~~ **Resolved 2026-05-21: `DATECONV-*` copybooks supplied; Risk 1 CLOSED**; surface DBIO dispatch enumeration. |
| 10 | Srinjoy | Does this tool construct a complete system-wide dependency graph (programs, copybooks, jobs) in the backend, or is analysis performed at an individual file level? | Use `analysis/dependency-map.md`; ~~identify missing `DATECONV-*` copybooks~~ **Resolved 2026-05-21: dependency graph closes end-to-end after customer follow-up shipment**; surface DBIO dispatch. |
...
| 16 | Margarita | What constructs are not supported or require manual remediation? | List unsupported/needs-review constructs; ~~mark missing copybooks~~ **Resolved 2026-05-21: previously-missing `DATECONV-*` copybooks supplied; only runtime-only behavior (dynamic dispatch, env-vars) remains.** |
